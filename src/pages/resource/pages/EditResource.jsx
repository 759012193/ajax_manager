...
class EditResource extends React.Component{
    ...
    componentDidMount() {
        ...
        if(this.props.location.state){
            ...
            if(resourceItem){
                ...
                // 2. 更新状态机
                this.setState({
                    imageUrl: resourceItem.resource_img, // 封面图
                    focusImageUrl: resourceItem.focus_img, // 首页焦点图
                    resourceId: resourceItem.id, // id编号
                    resourceContentTag: resourceItem.resource_content
                });
            }
    ...
    render() {
        ...
        const onFinish = (values) => {
            // 1. 处理数据
            const {imageUrl, focusImageUrl, dragFileList, resourceId, resourceContentTag} = this.state;
            if(!imageUrl){
            ...
            // 2. 上传资源
            editResource(getAdmin().token, resourceId,  values.resource_name, values.resource_author, resource_publish_time, dragFileList, values.resource_category_id, values.resource_classes_id, values.resource_area_id, values.resource_meta_id, values.resource_format_id, imageUrl, values.resource_price,  focusImageUrl, resourceContentTag).then((result)=>{
                if(result.data && result.data.status === 1){
                ...
                message.error('编辑幼教资源失败!', 1);
            });
        };


        const {resourceClasses, resourceMeta, resourceArea, resourceFormat, resourceCategory, imageUrl, focusImageUrl, dragFileList} = this.state;
        return (
            <Card title="编辑幼教资源">
                <Form
                    ref={this.resourceFormRef}
                    {...layout}
                    onFinish={onFinish}
                >
                    <Form.Item
