 import React, {Component} from "react"
  ...
  // 引入富文本编辑器
  import RichTextEdit from "./../../../components/YQRichTextEditor"
  // 引入tag选择
  import ActivitiesTag from './../../../components/YQTag'
  ...
  export default class AddActivities extends Component {
      ...
      componentDidMount() {
          ...
          getActivitiesBus().then((result)=>{
              if(result.data && result.data.status === 1){
                  this.setState({
                      activities_bus: result.data.data
                  })
              }
          }).catch((error)=>{
              console.log(error);
          ...
          getActivitiesAddress().then((result)=>{
              if(result.data && result.data.status === 1){
                  this.setState({
                      activities_address: result.data.data
                  })
              }
          }).catch((error)=>{
              console.log(error);
          ...
          getActivitiesObject().then((result)=>{
              if(result.data && result.data.status === 1){
                  this.setState({
                      activities_object: result.data.data
                  })
              }
          }).catch((error)=>{
              console.log(error);
      ...
      render() {
          const onFinish = values => {
              ...
              // 4. 调用接口
              editActivities(getAdmin().token, activities_id, values.activities_name, activities_time, imageUrl, values.activities_price, tagStr, values.activities_address_id, values.activities_object_id, values.activities_bus_day_id, activities_intro, activities_trip, activities_days, activities_notice, focusImgUrl).then((result)=>{
                  if(result.data && result.data.status === 1){
